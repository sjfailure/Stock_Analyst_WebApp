...
def detail_view_data_collector(company_id, category_id, period):
    ...
    logging.debug(f'start of detail_view_data_collector(): current time = {str(x)}')
    ...
    ).prefetch_related('date').order_by('-date__date')
    y = datetime.datetime.now()
    logging.debug(f'middle of detail_view_data_collector(): time since start = {str(y - x)}')
    return_data = package_json_data(data_dump, category_id)
    logging.debug(f'end of detail_view_data_collector(): DB call time:{str(y - x)} -- data_packaging time: {str(datetime.datetime.now() - y)}')
    return return_data
